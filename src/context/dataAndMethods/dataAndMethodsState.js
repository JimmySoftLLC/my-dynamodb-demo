...
const DataAndMethodsState = (props) => {
  ...
  const putItemDynamoDB = async (TableName, team_id, team_name, team_data) => {
    try {
      ...
      alertDialogContext.setAlertDialog(
        true,
        'Put not completed because this team is write protected.',
        'Error'
      );
    }
  };
  ...
  const setLoading = (myBool) =>
    dispatch({ type: SET_LOADING, payload: myBool });

  const setOnMyTeamPage = (myBool) =>
    dispatch({ type: SET_ON_MY_TEAM_PAGE, payload: myBool });

  const setRedirectTo = (myVal) =>
    dispatch({ type: SET_REDIRECT_TO, payload: myVal });

  const setEmail_to = (myVal) =>
    dispatch({ type: SET_EMAIL_TO, payload: myVal });

  const setEmail_subject = (myVal) =>
    dispatch({ type: SET_EMAIL_SUBJECT, payload: myVal });

  const setEmail_bcc = (myVal) =>
    dispatch({ type: SET_EMAIL_BCC, payload: myVal });

  const setEmail_body = (myVal) =>
    dispatch({ type: SET_EMAIL_BODY, payload: myVal });

  const setEmail_cc = (myVal) =>
    dispatch({ type: SET_EMAIL_CC, payload: myVal });

  const setMy_Teams = (myTeams) =>
    dispatch({ type: SET_MY_TEAMS, payload: myTeams });

  const setTeam_id = (myval) => dispatch({ type: SET_TEAM_ID, payload: myval });

  const setTeam_name = (myval) =>
    dispatch({ type: SET_TEAM_NAME, payload: myval });

  const setTeam_data = (myval) =>
    dispatch({ type: SET_TEAM_DATA, payload: myval });

  const setAmazonResponse = (myval) =>
    dispatch({ type: SET_AMAZON_RESPONSE, payload: myval });
